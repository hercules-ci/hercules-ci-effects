...
in
{
  ...
  config =
    ...
    in
    {
      herculesCI = mkIf enable (
        herculesCI@{ config, ... }:
        let
          releaseSystems = if cfg.systems == null then config.ciSystems else cfg.systems;
          systemFiles = lib.optionals (opt.filesPerSystem.isDefined) (
            lib.concatMap (system: withSystem system cfg.filesPerSystem) releaseSystems
          );
          ...
          onPush = mkMerge [
            {
              ${cfg.pushJobName}.outputs.effects.github-releases =
                lib.optionalAttrs (cfg.condition herculesCI.config.repo) deploy;
            }
            {
