...
let
  ...
  merge =
    t: vs:
    t.merge [ "test" "location" ] (
      lib.imap (i: v: {
        value = v;
        file = "test value number ${toString i}";
      }) vs
    );

  pkg = name: {
    type = "derivation";
    outPath = builtins.toFile "${name}" "";
    drvPath = builtins.toFile "${name}-drv" "kind of";
  };
  ...
  tests =
    ...
    assert
      merge derivationTree ex1 == {
        a = pkg "a";
        b = pkg "b";
      };
    assert
      (merge derivationTree ex2).n == {
        a = pkg "a";
        b = pkg "b";
      };
    assert
      (merge derivationTree ex2_1).n == lib.recurseIntoAttrs {
        a = pkg "a";
        b = pkg "b";
      };
    assert
      (merge derivationTree ex2_2).n == {
        a = pkg "a";
        b = pkg "b";
        c = null;
      };
    assert
      (merge derivationTree ex2_3).n == {
        a = pkg "a";
        b = pkg "b";
        c = { };
      };
