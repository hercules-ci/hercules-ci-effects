      ...
      let
        ...
        checkVersion = import ../effects/lib-version-check.nix {
          ...
          revInfo =
            ...
            if toString pkgs.path == toString inputs.nixpkgs.outPath or null && inputs ? nixpkgs.rev then
              " (rev: ${inputs.nixpkgs.rev})"
            else
              "";
          versionSource =
            if options.herculesCIEffects.pkgs.highestPrio < (lib.mkOptionDefault { }).priority then
              "from the perSystem option `herculesCIEffects.pkgs`"
            else
              "from the perSystem `pkgs` module argument";
