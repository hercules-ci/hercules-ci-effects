...
}:
{
  ...
  flake = {
    ...
    checks.x86_64-linux = {
      ...
      evaluation-runNixOS =
        let
          it = (import ./effects/nixos/eval-test.nix { inherit inputs; });
        in
        it.tests inputs.nixpkgs.legacyPackages.x86_64-linux.emptyFile // { debug = it; };

      evaluation-herculesCI =
        let
          it = (import ./flake-modules/herculesCI-eval-test.nix { inherit inputs; });
        in
        it.tests inputs.nixpkgs.legacyPackages.x86_64-linux.emptyFile // { debug = it; };

      evaluation-flake-update =
        let
          it = (import ./effects/flake-update/test-module-eval.nix { inherit inputs; });
        in
        it.tests inputs.nixpkgs.legacyPackages.x86_64-linux.emptyFile // { debug = it; };

      evaluation-mkHerculesCI =
        let
          it = (import ./lib/mkHerculesCI-test.nix { inherit inputs; });
        in
        it.tests inputs.nixpkgs.legacyPackages.x86_64-linux.emptyFile // { debug = it; };

      evaluation-nix-darwin =
        let
          it = (import ./effects/nix-darwin/eval-test.nix { inherit inputs; });
        in
        it.tests inputs.nixpkgs.legacyPackages.x86_64-linux.emptyFile // { debug = it; };
      ...
      evaluation-cargoPublish =
        let
          it = (
            import ./effects/cargo/eval-test.nix (
    ...
    tests = withSystem "x86_64-linux" (
      ...
      }:
      {
        ...
        nixops2 = pkgs.callPackage ./effects/nixops2/test/default.nix {
          nixpkgsFlake = inputs.nixpkgs-nixops2;
          pkgs = inputs.nixpkgs-nixops2.legacyPackages.${system};
        };
        ...
        outputs.effects =
          ...
          in
          {
            tests = top.config.flake.tests // {
              ...
              dev-kvm = withSystem "x86_64-linux" (
  ...
  perSystem =
    ...
    }:
    {
      ...
      checks =
        let
          github-releases-tests = import ./flake-modules/github-releases/test.nix {
            effectSystem = system;
            inherit inputs;
          };
          checkModules = builtins.deepSeq (lib.mapAttrs (_name: builtins.readFile) (self.modules.effect));
        in
        {
          ...
          flake-update-nix-git = hci-effects.callPackage ./effects/flake-update/test.nix {
