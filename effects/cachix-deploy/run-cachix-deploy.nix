...
args@{
  deploy ? throw "effects.runCachixDeploy: you must provide either a deploy or deployJsonFile argument.",
...
let
  checked =
    if args ? deploy && args ? deployJsonFile then
      throw "effects.runCachixDeploy: you must set either `deploy` or `deployJsonFile`."
    else
      x: x;
in
checked (
  mkEffect (
    ...
    // {
      ...
      effectScript =
        ...
        '';
      secretsMap = {
        activate = "default-cachix-activate";
      }
      // args.secretsMap or { };
      passthru = {
        inherit cachix;
