...
rec {
    ...
    flake-parts.lib.mkFlake { inherit inputs; } (
      { withSystem, self, ... }:
      {
        ...
        flake = {
          ...
          test.by-config = withSystem "x86_64-linux" (
            { hci-effects, ... }:
          ...
          test.by-config-legacy = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
          ...
          test.by-config-legacy-2 = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
          ...
          test.by-config-buildOnDestination = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
          ...
          test.by-config-buildOnDestination-override = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
          ...
          test.by-config-no-buildOnDestination = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
          ...
          test.by-config-no-ssh-buildOnDestination = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
              buildOnDestination = false;
          ...
          test.by-other-args = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
              system = "x86_64-darwin";
          ...
          test.by-other-args-pkgs = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
              system = "x86_64-darwin";
              nix-darwin = darwin.outPath;
              pkgs = darwin.inputs.nixpkgs.legacyPackages.x86_64-darwin.extend (
                self: super: { proof-of-overlay = "yes, overlay"; }
              );
              configuration = ./test/configuration.nix;
            }
          );
          test.by-other-args-buildOnDestination = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
              buildOnDestination = true;
          ...
          test.by-other-args-buildOnDestination2 = withSystem "x86_64-linux" (
            { hci-effects, ... }:
            hci-effects.runNixDarwin {
              ssh.destination = "john.local";
              ssh.buildOnDestination = true;
  ...
  tests =
    ...
    # Assumption
    assert
      flake1.darwinConfigurations."Johns-MacBook".system
      == flake1.darwinConfigurations."Johns-MacBook".config.system.build.toplevel;

    assert
      flake1.darwinConfigurations."Johns-MacBook".system.drvPath
      == flake1.test.by-config.config.system.build.toplevel.drvPath;

    # produces flake vs non-flake config
    # assert
    #   testEqDrv flake1.test.by-config.drvPath flake1.test.by-other-args.drvPath;

    assert testEqDrv flake1.test.by-config.drvPath flake1.test.by-config-legacy.drvPath;

    assert testEqDrv flake1.test.by-config.drvPath flake1.test.by-config-legacy-2.drvPath;

    assert builtins.isString flake1.test.by-other-args-pkgs.drvPath;
    ...
    assert testEqDrv flake1.test.by-other-args-pkgs.config.expose.pkgs.hello.drvPath
      darwin.inputs.nixpkgs.legacyPackages.x86_64-darwin.hello.drvPath;

    # A custom pkgs should be used without reinvoking nixpkgs from scratch.
    assert flake1.test.by-other-args-pkgs.config.expose.pkgs.proof-of-overlay == "yes, overlay";

    assert testEqDrv flake1.test.by-config.drvPath flake1.test.by-config-no-buildOnDestination.drvPath;

    assert testEqDrv flake1.test.by-config.drvPath
      flake1.test.by-config-no-ssh-buildOnDestination.drvPath;

    assert testEqDrv flake1.test.by-config-buildOnDestination.drvPath
      flake1.test.by-config-buildOnDestination-override.drvPath;

    assert testEqDrv flake1.test.by-other-args-buildOnDestination.drvPath
