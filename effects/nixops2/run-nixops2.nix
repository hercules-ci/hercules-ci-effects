...
let
  ...
  getNixFiles =
    nixops:
    runCommand "${nixops.name}-nix-files"
      {
        plugins =
          nixops.selectedPlugins
            or (if lib.isList nixops.plugins then nixops.plugins else lib.attrValues nixops.plugins);
        inherit nixops;
      }
      ''
        mkdir $out
    ...
    let
          ...
          if flake == null then
            {
              ...
              ++ map (v: origSrc + "/${v}") (networkFiles ++ prebuildOnlyNetworkFiles)
              ++ prebuildOnlyModules;
            }
          else
            {
      ...
      withChecks' =
        ...
        errorIf (!info ? network.storage || info ? network.storage.legacy)
          ...
          (
            lib.warnIf (!info ? network.lock || info.network.lock == { } || info ? network.lock.noop)
...
throwIfNot
  (
    action == "switch"
    || (
      args ? makeAnException
      &&
        args.makeAnException
        == "I know this can corrupt the state, until https://github.com/NixOS/nixops/issues/1499 is resolved."
    )
  )
  ...
  (
    ...
    // lib.filterAttrs (
      k: v: k != "prebuildNetworkArgs" && k != "prebuildOnlyModules" && k != "flake"
    ) args
