      ...
      nixopsConfigurations = {
        default = {
          ...
          defaults =
            { config, ... }:
            {
              nixpkgs.pkgs = perSystem.pkgs.${config.nixpkgs.system};
            };
            ...
            }:
            {
              config = {
                ...
                services.nginx.virtualHosts."${
                  if config.networking.publicIPv4 == null then "ip-unknown" else config.networking.publicIPv4
                }".root =
                  pkgs.myPkgs.my-web-root;
