...
rec {
  ...
  basicUpdateOutputs = callFlakeOutputs (
    inputs:
  ...
  basicUpdateConfig =
    basicUpdateHerculesCI.onSchedule.flake-update.outputs.effects.flake-update.config;

  subflakeUpdateOutputs = callFlakeOutputs (
    inputs:
    flake-parts.lib.mkFlake { inherit inputs; } (
      { ... }:
      {
  ...
  subflakeUpdateConfig =
    subflakeUpdateHerculesCI.onSchedule.flake-update.outputs.effects.flake-update.config;
  ...
  tests =
    ...
    assert
      basicUpdateHerculesCI.onSchedule.flake-update.when == {
        dayOfMonth = null;
        dayOfWeek = null;
        hour = [ 23 ];
        minute = 59;
